...
export default class SearchComponent extends Vue {
            ...
            .then(data => {
                ...
                fetch('api/search/GetOptions')
                    ...
                    .then(data => {
                        ...
                        if (profileUrl) {
                            var profileId = this.profiles.filter(profile => profile.name.toLowerCase() == profileUrl.toLowerCase())[0].id
                            this.profileFilter(profileId);
                        } else
                            if (get("search")) {
                                var cookie = getJSON("search")! as Cookie;
                                this.searchViewModel.companies.forEach(company => { company.selected = cookie.companies.indexOf(company.id) >= 0 });
                                this.searchViewModel.databases.forEach(company => { company.selected = cookie.databases.indexOf(company.id) >= 0 });
                                this.searchViewModel.websites.forEach(company => { company.selected = cookie.websites.indexOf(company.id) >= 0 });
                                this.searchViewModel.modes.forEach(company => { company.selected = cookie.modes.indexOf(company.id) >= 0 });
                                this.searchViewModel.purposes.forEach(company => { company.selected = cookie.purposes.indexOf(company.id) >= 0 });
    ...
    saveProfile() {
        var saveProfileModel: ProfileViewModel = {
            id: 0,
            name: this.profileName.replace(/\s/g, ''),
